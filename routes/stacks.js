...
router.get("/lefilter/?", (req, res, next) => {
  console.log(req.query.time)
  let { time, cat, src } = req.query
  let queries = []

  if (time !== undefined) {
    time = time.split(",").map(time => +time)
    queries.push({ timeInHours: { $in: time } })
  }
  if (cat !== undefined) {
    cat = cat.split(",")
    queries.push({ category: { $in: cat } })
  }
  if (src !== undefined) {
    src = time.split(",")
    queries.push({ $in: src })
  ...
  let theQuery = { $and: queries }

  console.log(JSON.stringify(theQuery))



  Stacks.find(theQuery)
    .sort({ "likesCounter": -1 })
    .lean()
    .then(allStacks =>
      res.render('stacks/show', {
